...
enum ScriptEvent {
    ...
    /// POCKETCOM: wheel scroll at logical (x, y) at a tick: re-positions the
    /// pointer (the app routes dy by the last mouse position) then sends dy
    /// in logical px — the same line the real wheel path emits.
...
struct Args {
    ...
    trace_frames: bool,
    /// POCKETCOM: PNG the window content at ticks (--screenshot PATH@TICK,
    /// repeatable; AppKit self-window capture, shot.rs — no Screen Recording
    /// permission).
...
fn parse_args_from<I: Iterator<Item = String>>(mut it: I) -> Result<Args> {
    ...
    while let Some(a) = it.next() {
        ...
        match a.as_str() {
            ...
            "--wheel" => {
                // POCKETCOM: --wheel X,Y,DY@TICK — scripted wheel scroll (dy
                // in logical px; the app's panel handler scrolls down for
                // negative dy, matching the real wheel's pixel delta).
                let v = val("--wheel")?;
                let (spec, t) = v
                    .rsplit_once('@')
            ...
            "--screenshot" => {
                // POCKETCOM: --screenshot PATH@TICK — dump the live window as
                // PNG at a tick (repeatable; AppKit self-window capture,
                // shot.rs).
