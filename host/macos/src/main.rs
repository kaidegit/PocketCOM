...
// POCKETCOM: self-developed serial bridge (SPEC §4.2). All new code lives in
...
impl PocketRoot {
    ...
    fn svc(&self, value: serde_json::Value) {
        ...
        if std::env::var_os("POCKETCOM_TRACE").is_some() {
            eprintln!("pocketcom-trace: svc -> {value}");
        }
        ...
        if self.forward_input() {
            ...
            if ks.modifiers.platform {
                ...
                match ks.key.as_str() {
                    ...
                    // Every other command chord (command-Enter send, …) goes to
                    // the guest as a cmd-flagged key line, mirroring the
                    // system-ui branch above.
                    k => self.svc(serde_json::json!({
                        "t": "key", "k": k, "cmd": true, "sh": shift,
                        "alt": ks.modifiers.alt, "ctl": ks.modifiers.control,
                    })),
