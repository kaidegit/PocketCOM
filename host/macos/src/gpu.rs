...
impl Target {
    ...
    /// POCKETCOM --screenshot support: read the rendered RGBA8 pixels back to
    /// the CPU. Runs on the calling (main) thread; the GPU wait is bounded by
    /// this frame's submission, already queued by the runtime worker.
    pub fn read_rgba(&self, gpu: &Gpu) -> Result<Vec<u8>> {
        let (w, h) = self.size;
        let bytes_per_row = ((w * 4 + 255) / 256) * 256;
        let staging = gpu.device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Pocket screenshot copy"),
        ...
        let mut encoder =
            gpu.device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("Pocket screenshot readback"),
                });
        ...
        let _ = gpu.device.poll(wgpu::PollType::WaitForSubmissionIndex(submission));
        rx.recv()
            .map_err(|_| anyhow!("screenshot readback channel closed"))?
            .map_err(|e| anyhow!("screenshot map failed: {e}"))?;
        let mut out = vec![0u8; (w * h * 4) as usize];
        {
            let data = &slice.get_mapped_range();
            for row in 0..h as usize {
                let src = &data[row * bytes_per_row as usize..][..w as usize * 4];
                out[row * w as usize * 4..][..src.len()].copy_from_slice(src);
            }
        }
        staging.unmap();
        Ok(out)
...
pub struct Renderer {
    ...
    /// POCKETCOM: frames actually submitted — the exit receipt's counter.
