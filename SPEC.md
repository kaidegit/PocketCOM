...
- ASCII / HEX 显示切换（HEX 大写、空格分隔；仅作用于 RX/TX 数据帧——系统消息 `[--]` 为 UI 生成的完整 UTF-8 文本，不受 HEX/转义开关影响，始终按文本显示）
...
- 缓冲：消息总线为**有界环形缓冲**（默认 1000 帧 / 256 KiB，可配），溢出丢最旧帧属正常历史裁剪，**不注入事件**。真实丢帧由消费端按消息 id 连续性检测（断档 = 有帧在消费前被裁）：UI 接收区记一条 i18n `sys` 提示（`[--] 缓冲溢出，已丢弃 N 帧`）；MCP 读行前置一条 `[SYS] buffer overflow, dropped N frame(s)`（稳定英文，§6.3）。规避 umeko_serial_mcp 缓冲无上限的缺陷。
